...
signal demux_OUT_in       : std_logic_vector(12 downto 0);
signal demux_OUT_out      : std_logic_vector(1 downto 0);
signal demux_out_addres   : std_logic_vector(1 downto 0);
...
demux_OUT_in <= write_ram & ram_address;
...
with ram_address select
    mux_IN_out <= sw                        when "0000000000000001",
                  "00000000000" & d_btn     when "0000000000000011",
                  seconds                   when "0000000000000100",
                  mseconds                  when "0000000000000101",
                  useconds                  when "0000000000000110",
                  ram_dataout               when others;

-- Demux Out

with ram_address select
    demux_out_addres <= "01" when "000000000000",
                        "10" when "000000000010",
                        "00" when others;

with write_ram select
    demux_OUT_out <= demux_out_addres when '1',
                     "00" when others;              
...
inst_RegDis: Reg port map(
    clock       => clock,
    clear       => clear,
    load        => demux_OUT_out(1), --demux out
    up          => '0',
    down        => '0',
    datain      => ram_datain,
    dataout     => dis --MUXA
 ...
 inst_RegLed: Reg port map(
    clock       => clock,
    clear       => clear,
    load        => demux_OUT_out(0), --demux out
    up          => '0',
    down        => '0',
    datain      => ram_datain,
    dataout     => led --MUXA
